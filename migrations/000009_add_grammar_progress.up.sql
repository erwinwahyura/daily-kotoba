...
ALTER TABLE user_progress ADD COLUMN last_grammar_id UUID REFERENCES grammar_patterns(id);

-- Create grammar status tracking table
CREATE TABLE user_grammar_status (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    grammar_id UUID NOT NULL REFERENCES grammar_patterns(id),
    status VARCHAR(20) NOT NULL CHECK (status IN ('learning', 'mastered', 'skipped')),
